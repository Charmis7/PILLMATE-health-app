// ════════════════════════════════════════════════════════════════════════════
// thrice_daily_screen.dart
// PURPOSE : Same flow as OnceDailyScreen but with THREE IntakeSlots.
// ════════════════════════════════════════════════════════════════════════════
...
class _ThreeTimesDailyScreenState extends State<ThreeTimesDailyScreen> {
  ...
  Future<void> _saveAndNavigate() async {
    ...
    try {
      ...
      // MEDICINE SERVICE: save to Firestore, get doc ID
      final docId = await MedicineService.saveMedicine(entry); // FIRESTORE: .add()

      // NOTIFICATION SERVICE: schedules THREE daily alarms (one per IntakeSlot)
            ...
            children: [
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      _doseRow(thirdDose,  (v) => setState(() => thirdDose  = v)),
                    ],
                  ),
                ),
              ),
              SizedBox(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF5D9CFF),
  ...
  Widget _sectionTitle(String t) => Align(alignment: Alignment.centerLeft,
      child: Padding(padding: const EdgeInsets.symmetric(vertical: 8),
          child: Text(t, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blueGrey, fontSize: 14))));
