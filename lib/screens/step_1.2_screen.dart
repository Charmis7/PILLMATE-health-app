...
class Step_2 extends StatefulWidget {
  ...
  const Step_2({super.key, required this.medicineName, required this.unit});
...
class _Step_2State extends State<Step_2> {
  ...
  void _navigate(String option) {
    ...
    if (option == "Once a daily") {
      nextScreen = OnceDailyScreen(medicineName: widget.medicineName, unit: widget.unit);
    } else if (option == "Twice a daily") {
      nextScreen = TwiceDailyScreen(medicineName: widget.medicineName, unit: widget.unit);
    } else if (option == "Three time a daily") {
      nextScreen = ThreeTimesDailyScreen(medicineName: widget.medicineName, unit: widget.unit);
    } else {
      nextScreen = AsNeededScreen(medicineName: widget.medicineName, unit: widget.unit);
    }
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                // --- LOGO (Hero for smooth transition) ---
                Hero(
                  tag: 'app_logo',
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          ...
                          // Brief delay so user sees the selection before navigating
                          Future.delayed(const Duration(milliseconds: 200), () => _navigate(option));
                        },
                        child: Container(
                          margin: const EdgeInsets.only(bottom: 16),
                          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 20),
                          decoration: BoxDecoration(
                            // Glass look: white with low opacity
                            color: isSelected
                            ...
                            boxShadow: isSelected ? [
                              BoxShadow(
                                color: const Color(0xFF5D9CFF).withOpacity(0.2),
                                blurRadius: 10,
